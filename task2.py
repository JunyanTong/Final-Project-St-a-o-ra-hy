...
# Mean and std deviation of  dataset. 
...
def gaussian(tensor, mean=0, stddev=0.1):
    ...
    noise = torch.nn.init.normal(torch.Tensor(tensor.size()), 0, 0.1).cuda()
...
class HidingNetwork(nn.Module):
    ...
    def forward(self, h):
        ...
        out_noise = gaussian(out.data, 0, 0.1).cuda()
...
def train_model(train_loader, beta, learning_rate):
    ...
    for epoch in range(num_epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            train_secrets = Variable(train_secrets, requires_grad=False).cuda()
            train_covers = Variable(train_covers, requires_grad=False).cuda()
...
for idx, test_batch in enumerate(test_loader):
    ...
    # Creates variable from secret and cover images
    test_secret = Variable(test_secret, volatile=True).cuda()
    test_cover = Variable(test_cover, volatile=True).cuda()
    ...
    print ("The {}th pair pictures' rmse is :{}".format(idx, test_loss.data))
